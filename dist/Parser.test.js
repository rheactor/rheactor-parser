...
describe("Parser class", () => {
    test("no rule specified", () => {
        expect(() => new Parser_1.Parser().parse("")).toThrow("no rule specified");
    });
    test("rule only parser", () => {
        ...
        expect(() => parser.parse("test 123")).toThrow('unexpected "123" at offset 5');
        expect(() => parser.parse("test123")).toThrow('unexpected "123" at offset 4');
    });
    ...
    test("rule transform, returning own", () => {
        const parser = new Parser_1.Parser();
        parser.rule("statement", /test/).transform((own) => own);
        expect(parser.parse("test")).toBe("test");
    });
    ...
    test("token is duplicated", () => {
        ...
        expect(() => new Parser_1.Parser().tokens("test", "test")).toThrow('token "test" already defined');
    });
    ...
    test("rule does not have a valid identifier", () => {
        expect(() => new Parser_1.Parser().rule("123", /123/)).toThrow('rule "123" does not have a valid identifier');
    });
    ...
    test("rule with an unknown term", () => {
        ...
        expect(() => parser.parse("a")).toThrow('unknown term "a" at rule "initial"');
    });
    test("rule with an unknown term, multiple implementations of rule", () => {
        const parser = new Parser_1.Parser();
        parser.rule("initial", ["a"]);
        parser.rule("initial", ["b"]);
        expect(() => parser.parse("a")).toThrow('unknown term "a" at rule "initial[0]"');
    });
    ...
    test("rule without terms", () => {
        const parser = new Parser_1.Parser();
        expect(() => parser.rule("initial", [])).toThrow('rule "initial" must define at least one term');
    });
    ...
    test("subrules with default transformations", () => {
        const parser = new Parser_1.Parser({ ruleInitial: "initial" });
        parser.token("a");
    ...
    test("consume must fail if validation fails, with custom message", () => {
        ...
        const expectedError = new ParserError_1.ParserError("number too big");
        ...
        try {
            ...
            /* eslint-disable */
            expect(e).toBeInstanceOf(ParserError_1.ParserError);
            if (e instanceof ParserError_1.ParserError) {
    ...
    test("ensure that validate() arguments are spread", () => {
        ...
        expect(() => parser.parse("12345 + abc")).toThrow("number must be lower than 1000");
    });
});
describe("README.me examples", () => {
    ...
    test("readme: recursive sum, bad definition", () => {
        ...
        parser.rule("sum", ["number", "+", "sum"]).transform((a, b) => a + b);
        parser.rule("number", /\d/).transform((d) => Number(d));
        expect(() => parser.parse("1+2+3+4+5")).toThrow('unexpected "1" at offset 0');
    });
